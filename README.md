# 證道輯錄：回歸真理、聖靈與文字的深度自省
# Sermon Collections: Reflection, Spirit, and the Power of Text
> 🌐 **網頁版閱讀中心：[https://shenenw.github.io/sermon-collection/](https://shenenw.github.io/sermon-collection/聪明人要做智慧的事情.html)**

- 「好的證道不應僅止於歌頌神的偉大。若缺乏反省，信仰將流於形式；若沒有聖靈，讀經將失去生命；若沒有文字的沉澱，真理難以扎根。」

---

## 📖 專案核心理念 (Core Philosophy)

本專案旨在收錄那些能引導信徒進入靈魂深處、產生真實變革的證道資源。

### 1. 文字的消化效率 (Textual Efficiency vs. Sequential Media)
目前教會多以影音（Video/Audio）紀錄證道，雖便於製作，但對接收者而言：
* **影音是線性的 (Sequential)**：聽眾必須跟隨講員的速度，難以即時停頓思索。
* **文字具備自主性**：文字允許讀者**隨時停頓、回溯、反覆咀嚼**。對於需要深度「反省」的真理，文字提供了消化與內化所需的空間。

### 2. 復興的先決條件：反省與拆毀
基督徒皆知神的美好，但生命的轉變始於**反省 (Self-examination)**。
* **摩西的警誡**：效法摩西在離世前的切切叮嚀（申命記 31章），直面人性的軟弱與悖逆。
* **希西家的改革**：如同希西家王打碎銅蛇，屬靈復興必先經歷自省與拆毀偶像，而非廉價的歌頌。

### 3. 聖靈與真理的同在
**「讀經固然重要，但若沒有神的靈同在，真理便無法進入人心。」**
讀經是根基，聖靈是動力，而反省則是聖靈在心中結出的果子。我們追求的是在聖靈引領下，讓經文成為剖開靈魂的利劍。

---

## 🛡️ 紀錄說明與免責聲明 (Notes & Disclaimer)

* **技術處理**：部分內容係透過 AI 工具（如 Whisper）由影音轉錄而成，旨在提高資訊檢索與研讀效率。
* **匿名原則**：出於對原講員隱私與意願的尊重，**部分收錄內容將不具名**。除非獲得明確授權，否則本專案僅保留文字精華，隱去具體姓名，以專注於真理訊息本身。
* **用途**：僅供個人研經、集體小組討論及靈命反省使用。

---

## 📂 收錄準則
1. **非線性思考**：適合文字閱讀與反覆查考。
2. **反省穿透力**：挑戰自我、拒絕盲目樂觀。
3. **聖靈同在感**：強調生命與神的直接連結。

---

**Note:** This collection is currently in the enrichment phase. 
Contact information and discussion features will be enabled once the initial repository is complete.

**File:** README.md  
**Project:** Sermon Collections  
**Last Updated:** 2026-04-01
